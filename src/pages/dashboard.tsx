...
import { getSelectedMp3Players } from 'apiSdk/selected-mp-3-players';
...
function DashboardPage() {
  ...
  const selectedMp3PlayersStat = useSWR<PaginatedInterface<SelectedMp3PlayersInterface>>(
    () => `/selected-mp-3-players`,
    () => getSelectedMp3Players({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Selected Mp 3 Players',
      result: selectedMp3PlayersStat?.data?.totalCount,
      isLoading: selectedMp3PlayersStat.isLoading,
